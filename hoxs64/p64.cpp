...
PP64RangeCoderProbabilities P64RangeCoderProbabilitiesAllocate(p64_uint32_t Count) {
	return (PP64RangeCoderProbabilities)p64_malloc(Count * sizeof(p64_uint32_t));
}
...
void P64RangeCoderWrite(PP64RangeCoder Instance, p64_uint8_t Value) {
	if(Instance->BufferPosition >= Instance->BufferSize) {
		...
		if(Instance->Buffer) {
			Instance->Buffer = (p64_uint8_t *)p64_realloc(Instance->Buffer, Instance->BufferSize);
		} else {
			Instance->Buffer = (p64_uint8_t *)p64_malloc(Instance->BufferSize);
		}
	}
...
p64_uint32_t P64MemoryStreamWrite(PP64MemoryStream Instance, p64_uint8_t* Data, p64_uint32_t Count) {
	if(Count) {
		if((Instance->Position + Count) >= Instance->Allocated) {
			...
			if(Instance->Data) {
				Instance->Data = (p64_uint8_t *)p64_realloc(Instance->Data, Instance->Allocated);
			} else {
				Instance->Data = (p64_uint8_t *)p64_malloc(Instance->Allocated);
			}
		}
...
p64_uint32_t P64MemoryStreamAssign(PP64MemoryStream Instance, PP64MemoryStream FromInstance) {
	...
	memset(Instance, 0, sizeof(TP64MemoryStream));
	Instance->Data = (p64_uint8_t *)p64_malloc(FromInstance->Allocated);
...
p64_int32_t P64PulseStreamAllocatePulse(PP64PulseStream Instance) {
	...
	if(Instance->FreeList < 0) {
		if(Instance->PulsesCount >= Instance->PulsesAllocated) {
			...
			if(Instance->Pulses) {
				Instance->Pulses = (PP64Pulses)p64_realloc(Instance->Pulses, Instance->PulsesAllocated * sizeof(TP64Pulse));
			} else {
				Instance->Pulses = (PP64Pulses)p64_malloc(Instance->PulsesAllocated * sizeof(TP64Pulse));
			}
		}
...
p64_uint32_t P64PulseStreamReadFromStream(PP64PulseStream Instance, PP64MemoryStream Stream) {
	...
	if(P64MemoryStreamReadDWord(Stream, &CountPulses)) {
		...
		if(P64MemoryStreamReadDWord(Stream, &Size)) {
			...
			Buffer = (p64_uint8_t *)p64_malloc(Size);
...
p64_uint32_t P64ImageReadFromStream(PP64Image Instance, PP64MemoryStream Stream) {
	...
	if(P64MemoryStreamSeek(Stream, 0) == 0) {
		if(P64MemoryStreamRead(Stream, (p64_uint8_t*)&HeaderSignature, sizeof(TP64HeaderSignature)) == sizeof(TP64HeaderSignature)) {
...
{ \
	...
	if (P64MemoryStreamWrite(&ChunksMemoryStream, (p64_uint8_t *)&ChunkSignature, sizeof(TP64ChunkSignature)) == sizeof(TP64ChunkSignature)) { \
	...
	if(result) {
		...
		if(result) {
			...
			if(P64MemoryStreamWrite(&MemoryStream, (p64_uint8_t *)&HeaderSignature, sizeof(TP64HeaderSignature)) == sizeof(TP64HeaderSignature)) {
