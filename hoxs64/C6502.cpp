...
void CPU6502::ExecuteCycle(ICLK sysclock)
{
	...
	while ((ICLKS)(sysclock - CurrentClock) > 0)
	{
		...
		case C_JSR_ABSOLUTE_5:
			CHECK_BA;
			mPC.byte.hiByte= ReadByte(mPC.word);
		...
		case LAS_ABSOLUTEY:
			CHECK_BA;
			mSP = mX = mA = (mSP & ReadByte(addr.word));
			fZERO= (mA==0);
			fNEGATIVE= (mA & 0x80) >> 7;
			check_interrupts1();
			m_cpu_sequence=C_FETCH_OPCODE;
