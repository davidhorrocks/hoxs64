...
void Sprite::Initialize(ID3D11Device* device, ID3D11DeviceContext* deviceContext, float width, float height, DXGI_FORMAT format, const Color& color, aiTextureType type, ConstantBuffer<CB_VS_vertexshader_2d>& cb_vs_vertexshader_2d)
{
	this->texture = std::make_unique<Texture>(device, format, color, aiTextureType::aiTextureType_DIFFUSE);
	Init(device, deviceContext, width, height, format, cb_vs_vertexshader_2d);
}

void Sprite::Initialize(ID3D11Device* device, ID3D11DeviceContext* deviceContext, float width, float height, DXGI_FORMAT format, std::wstring spritePath, ConstantBuffer<CB_VS_vertexshader_2d>& cb_vs_vertexshader_2d)
{
	this->texture = std::make_unique<Texture>(device, format, spritePath, aiTextureType::aiTextureType_DIFFUSE);
	Init(device, deviceContext, width, height, format, cb_vs_vertexshader_2d);
}

void Sprite::Init(ID3D11Device* device, ID3D11DeviceContext* deviceContext, float width, float height, DXGI_FORMAT format, ConstantBuffer<CB_VS_vertexshader_2d>& cb_vs_vertexshader_2d)
{
