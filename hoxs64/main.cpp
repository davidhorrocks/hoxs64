...
HRESULT CApp::InitInstance(int nCmdShow, PWSTR lpCmdLine)
{
	...
	CommandArg* caMountCrt = cmdArgs.FindOption(TEXT("-mountcrt"));
	...
	bool keepCurrentCrt = false;
	if (caMountCrt != nullptr && caMountCrt->ParamCount >= 1)
	{
		if (SUCCEEDED(c64.LoadCrtFile(caMountCrt->pParam[0])))
		{
			keepCurrentCrt = true;
		}
	}
	...
	if (caAutoLoad)
	{
		if (caAutoLoad->ParamCount >= 1)
		{
			...
			c64.AutoLoad(pFilename, directoryIndex, indexPrgsOnly, pC64filename, wantQuickLoad, alignD64, keepCurrentCrt, caReu512k != nullptr);
		}
	}
