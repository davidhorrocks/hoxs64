...
HRESULT C64::AutoLoad(const TCHAR *filename, int directoryIndex, bool bIndexOnlyPrgFiles, const bit8 c64FileName[C64DISKFILENAMELENGTH], bool bQuickLoad, bool bAlignD64Tracks, bool keepCurrentCrt, bool bReu)
{
	HRESULT hr = S_OK;
	C64File c64file;
...
HRESULT C64::LoadImageFile(const TCHAR* filename, bool injectC64Memory, bit16* pStartAddress, bit16* injectedC64Size, bit8* pBuffer, bit32* pcbBufferSize)
{
	try
	{
		...
		if (pBuffer == nullptr)
		{
			...
			{
				*pcbBufferSize = (bit32)file_size;
			}
		}

		if (pBuffer != nullptr || injectC64Memory)
		{
			auto pTempBuffer = std::shared_ptr<bit8[]>(new bit8[file_size]);
			bytes_to_read = (bit32)file_size;
