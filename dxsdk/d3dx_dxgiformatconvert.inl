...
#define hlsl_precise
	...
	{
		return std::min(std::max(_V, 0.0f), 1.0f);
	}
	...
	{
		return (FLOAT)(_V >= 0 ? floor(_V) : ceil(_V));
	}
	...
	D3DX11INLINE FLOAT D3DX_SRGB_to_FLOAT_inexact(hlsl_precise FLOAT val)
	{
		if (val < 0.04045f)
			...
			val = (FLOAT)pow((val + 0.055f) / 1.055f, 2.4f);
		return val;
	}
	...
	D3DX11INLINE FLOAT D3DX_FLOAT_to_SRGB(hlsl_precise FLOAT val)
	{
		if (val < 0.0031308f)
			...
			val = (FLOAT)(1.055f * pow(val, 1.0f / 2.4f) - 0.055f);
		return val;
	}
	...
	D3DX11INLINE FLOAT D3DX_SaturateSigned_FLOAT(FLOAT _V)
	{
		...
		return std::min(std::max(_V, -1.0f), 1.0f);
	}
	...
	{
		UINT packedOutput;
		unpackedInput.x = std::min(unpackedInput.x, (uint32_t)0x000003ff);
		unpackedInput.y = std::min(unpackedInput.y, (uint32_t)0x000003ff);
		unpackedInput.z = std::min(unpackedInput.z, (uint32_t)0x000003ff);
		unpackedInput.w = std::min(unpackedInput.w, (uint32_t)0x00000003);
	...
	{
		UINT packedOutput;
		unpackedInput.x = std::min(unpackedInput.x, (uint32_t)0x000000ff);
		unpackedInput.y = std::min(unpackedInput.y, (uint32_t)0x000000ff);
		unpackedInput.z = std::min(unpackedInput.z, (uint32_t)0x000000ff);
		unpackedInput.w = std::min(unpackedInput.w, (uint32_t)0x000000ff);
		packedOutput = (unpackedInput.x |
			(unpackedInput.y << 8) |
			(unpackedInput.z << 16) |
			(unpackedInput.w << 24));
	...
	{
		UINT packedOutput;
		unpackedInput.x = std::max(std::min(unpackedInput.x, 127), -128);
		unpackedInput.y = std::max(std::min(unpackedInput.y, 127), -128);
		unpackedInput.z = std::max(std::min(unpackedInput.z, 127), -128);
		unpackedInput.w = std::max(std::min(unpackedInput.w, 127), -128);
	...
	{
		UINT packedOutput;
		unpackedInput.x = std::min(unpackedInput.x, (uint32_t)0x0000ffff);
		unpackedInput.y = std::min(unpackedInput.y, (uint32_t)0x0000ffff);
		packedOutput = (unpackedInput.x |
			(unpackedInput.y << 16));
		return packedOutput;
	}
	...
	{
		UINT packedOutput;
		unpackedInput.x = std::max(std::min(unpackedInput.x, 32767), -32768);
		unpackedInput.y = std::max(std::min(unpackedInput.y, 32767), -32768);
